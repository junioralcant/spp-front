...
export default function ListNotaDespesaRoco({ history, match }) {
  ...
  useEffect(() => {
    async function loadExpenseNotes(page = numberPage) {
      const response = await api.get(`/notasdespesasrocos?page=${page}`);
  ...
  async function destroy(id) {
    await api.delete(`/notasdespesasrocos/${id}`);
    const response = await api.get("/notasdespesasrocos");
  ...
  async function filterWhoBought(e) {
    if (e.target.value !== "") {
      const response = await api.get(
        `/notasdespesasrocos?nome_quem_comprou=${e.target.value}&limit_page=${expenseNotesRest.total}`
      );
      const { docs, ...restExpense } = response.data;
      setExpense(response.data.docs);
      setExpenseRest(restExpense);
  ...
  async function filterExpenseOn(e) {
    if (e.target.value !== "") {
      const response = await api.get(
        `/notasdespesasrocos?gastocom=${e.target.value}&limit_page=${expenseNotesRest.total}`
      );
      const { docs, ...restExpense } = response.data;
      setExpense(response.data.docs);
      setExpenseRest(restExpense);
  ...
  async function filterNameStore(e) {
    if (e.target.value !== "") {
      const response = await api.get(
        `/notasdespesasrocos?nome_loja=${e.target.value}&limit_page=${expenseNotesRest.total}`
      );
      const { docs, ...restExpense } = response.data;
      setExpense(response.data.docs);
      setExpenseRest(restExpense);
  ...
  async function filterData() {
    if (dataMin !== "" || dataMax !== "") {
      const response = await api.get(
        `/notasdespesasrocos?data_min=${dataMin}&data_max=${dataMax}&limit_page=${expenseNotesRest.total}`
      );
              ...
              {expenseNotes.map(note => {
                ...
                return (
                  <tr key={note._id}>
                    ...
                    <td>{note.total} R$</td>
                    <td>
                      <Link to={`/notadespesaroco/${note._id}`}>
                        <IoMdCreate />
                      </Link>
                      <Link
                        to="#"
