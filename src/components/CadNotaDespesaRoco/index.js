...
export default function CadNotaDespesaRoco({ history, match }) {
    ...
    if (!match.params.id) {
      if (
        ...
        try {
          ...
          await api.postOrPut("/notasdespesasrocos", match.params.id, data);
  ...
  useEffect(() => {
    async function loadData() {
      if (match.params.id) {
        ...
        const response = await api.get(`/notasdespesasrocos/${id}`);
  ...
  return (
    <Container>
      <Form initialData={data} onSubmit={handlerSubmit}>
        <h1>Cadastro de Despesa do Roço</h1>
        <ContentForm>
          <div>
