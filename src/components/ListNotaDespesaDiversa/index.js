...
export default function ListNotaDespesaDiversa({ history, match }) {
  ...
  useEffect(() => {
    async function loadExpenseNotes(page = numberPage) {
      const response = await api.get(`/notasdespesasdiversas?page=${page}`);
  ...
  async function destroy(id) {
    await api.delete(`/notasdespesasdiversas/${id}`);
    const response = await api.get("/notasdespesasdiversas");
  ...
  async function filterWhoBought(e) {
    if (e.target.value !== "") {
      const response = await api.get(
        `/notasdespesasdiversas?nome_quem_comprou=${e.target.value}&limit_page=${expenseNotesRest.total}`
      );
      const { docs, ...restExpense } = response.data;
      setExpense(response.data.docs);
      setExpenseRest(restExpense);
  ...
  async function filterExpenseOn(e) {
    if (e.target.value !== "") {
      const response = await api.get(
        `/notasdespesasdiversas?gastocom=${e.target.value}&limit_page=${expenseNotesRest.total}`
      );
      const { docs, ...restExpense } = response.data;
      setExpense(response.data.docs);
      setExpenseRest(restExpense);
  ...
  async function filterNameStore(e) {
    if (e.target.value !== "") {
      const response = await api.get(
        `/notasdespesasdiversas?nome_loja=${e.target.value}&limit_page=${expenseNotesRest.total}`
      );
      const { docs, ...restExpense } = response.data;
      setExpense(response.data.docs);
      setExpenseRest(restExpense);
  ...
  async function filterData() {
    if (dataMin !== "" || dataMax !== "") {
      const response = await api.get(
        `/notasdespesasdiversas?data_min=${dataMin}&data_max=${dataMax}&limit_page=${expenseNotesRest.total}`
      );
  ...
  return (
    <ContainerList>
      <ContentList>
        <h1>Gastos Despesas Diversas</h1>
        <Pesquisa>
          <input
              ...
              {expenseNotes.map(note => {
                ...
                return (
                  <tr key={note._id}>
                    ...
                    <td>{note.total} R$</td>
                    <td>
                      <Link to={`/notadespesadiversa/${note._id}`}>
                        <IoMdCreate />
                      </Link>
                      <Link
                        to="#"
